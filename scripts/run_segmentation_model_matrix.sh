...
dataset_label_for_dataset() {
  local dataset="$1"
  printf '%s' "${dataset%_segmentation}"
}
...
for dataset in "${DATASET_LIST[@]}"; do
  for optimizer in "${OPTIMIZER_LIST[@]}"; do
    for model in "${MODEL_LIST[@]}"; do
      dataset_label="$(dataset_label_for_dataset "${dataset}")"
      model_type="$(model_type_for_model "${model}")"
      model_size="$(model_size_for_model "${model}")"
      run_model_label="${model%_cuda}"
      RUN_INDEX=$((RUN_INDEX + 1))
      if [ "${RUN_INDEX}" -le "${SKIP_FIRST_N}" ]; then
        echo "Skipping ${RUN_INDEX}: ${dataset}_${model}_${optimizer}"
        continue
      ...
      run_name="seg_${dataset_label}_${run_model_label}"
      ...
      CMD=(
        ...
        model.img_size="${MODEL_IMG_SIZE}"
        "${TRAINER_ARGS[@]}"
        "${DATAMODULE_ARGS[@]}"
        logger.project="${WANDB_PROJECT}"
        logger.name="${run_name}"
        "logger.extra_tags=['dataset:${dataset_label}','model_type:${model_type}','model_size:${model_size}']"
      )
